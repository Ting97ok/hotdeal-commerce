...
3방식(낙관락 · 조건부 UPDATE · Redis+Lua)을 **같은 워크로드로 전략만 바꿔 재실행**해 비교한다.
...
3. **앱 기동 (전략 선택)**
   ```bash
   ./gradlew bootRun --args='--stock.deduct.strategy=optimistic'   # 또는 conditional / redis
   ```
...
6. **3방식 반복** — 전략만 바꿔 2~5 재실행 → 결과를 [ADR-0010] 에 정리
