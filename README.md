...
- **스케일아웃의 한계** — LB 뒤 앱 1 vs 3 실측으로 "재고 처리량은 서버를 늘려도 안 늘고 오히려 악화"를 수치로(정확성·커넥션 총량은 인스턴스 수와 무관함도 확인)
...
### ① 스케일아웃해도 재고 병목은 안 풀린다 — 앱 1대 vs 3대

"서버를 늘리면 되잖아"를 실측으로 반박한다. nginx 로드밸런서 뒤 앱을 1대→3대로 늘려 같은 부하를 줬다.

![인스턴스 1→3: 동률에서 출발해 conditional 만 4.4s 로 급증, redis 는 2.6s](docs/design/images/benchmark-multi-instance.svg)

- **처리량은 안 늘고 지연만 악화** — 조건부 UPDATE p95 가 3대에서 **4.4s**. 행 잠금 대기는 7.5s → **71.6s** 로 약 10배.
- **자원 부족이 아니다** — 커넥션 31개가 물렸는데 실제 실행 중은 **2개뿐**. 자원이 남는데도 **한 줄로 서서** 처리되는 구조(**한 줄 병목**)라 서버 증설로는 안 풀린다. 해법은 인프라가 아니라 데이터 모델.
- **정확성·커넥션 산정은 인스턴스 수와 무관** — 3대에서도 성공 정확히 10 · 오버셀 **0**, 커넥션 합산 30 = DB `Threads_connected` 31.
- **정직한 한계** — 로컬 1머신이라 악화 원인을 한 줄 병목과 CPU 경합으로 **분리하지 못했다**. 물리 분리 측정이 후속.

### ② 전략 선정 — 같은 워크로드로 3전략 실측

동시 1,000명이 재고 한 행을 다투는 오픈 스파이크를 낙관락 · 조건부 UPDATE · Redis+Lua 로 측정해 **원자적 조건부 UPDATE** 를 운영 전략으로 확정했다 — [ADR-0010](docs/adr/0010-concurrency-strategy-selection.md).
