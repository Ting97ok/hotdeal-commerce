...
토스는 `TossPayments-Test-Code` 헤더로 특정 에러를 강제할 수 있어, 코드별로 어떤 상태값이 오는지 샌드박스에 직접 물어 확인했다. 물어보는 [스크립트](evidence/capture-toss-errors.sh)와 받은 [응답 원본](evidence/toss-error-responses.log)을 남겨 두었다.
...
| `UNAPPROVED_ORDER_ID` | 400 | 승인 전이라 돈이 안 빠진 확정 거절 |
...
- **토스가 응답을 바꾸면 알아차리지 못한다.** 5절 표는 응답 원본과 어댑터 테스트 양쪽에 고정돼 있지만, 바뀐 것을 알려면 스크립트를 다시 돌려 대조해야 한다. 자동으로 감시하지는 않는다
