...
- 앱 3대에서 조건부 2.52초와 Redis 1.99초의 차이인 **0.53초가 락 경합이 추가로 만든 몫**이다
...
- **자원 경합의 몫을 정밀하게 나누지 못했다.** Redis 대조로 락 경합분(0.53초)은 갈라냈지만, 나머지가 CPU인지 네트워크인지 컨테이너 스케줄링인지는 모른다. 부하 도구·앱·DB를 각각 다른 기기에 두어야 가려지는데 장비 여건상 확인하지 못했다
