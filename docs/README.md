...
> 트래픽이 한순간에 몰리는 **핫딜 선착순 구매**를, 재고보다 많이 팔리는 일(초과 판매) 0건으로 막고 토스 결제까지 정합성 있게 잇는 백엔드 프로젝트. 넓은 기능 대신 **이 한 흐름을 끝까지 깊게** 판다(적은 API + 깊이).
...
| **DB FK 제약 미사용** | 폭주 쓰기의 부모 행 락 제거 + 결제 후속 MSA 분리 대비 | [ADR-0003](adr/0003-no-db-fk-constraints.md) |
...
| 5 | [ADR](adr/README.md) → 관심 결정 | 각 결정의 대안·트레이드오프. 추천: [주문 — 선점·복원·불변식](adr/order.md) · [재고 동시성](adr/concurrency.md) · [결제 모델·PG 경계](adr/0008-payment-model-pg-boundary.md) |
