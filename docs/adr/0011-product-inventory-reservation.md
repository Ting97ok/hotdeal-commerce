...
상품별 별도 테이블의 한 행(칼럼 적음): `product_id`(UNIQUE 논리 참조) · `on_hand_quantity`(실물 — 창고에 실제 있는 수) · `reserved_quantity`(예약 — 핫딜에 떼어 둔 수) · `version`(낙관락 버전 — 동시 수정 충돌 감지).
...
- **주문 모델 = 상품 참조 + 적용 핫딜 기록 + 금액 스냅샷** (표준 강제): 커머스 주문의 라인아이템은 상품을 참조하고 적용 프로모션 id 와 주문 시점 가격을 함께 보존한다(Salesforce B2C `ProductLineItem`·commercetools·Craft Commerce). → `Order` 는 `product_id`(무엇을 샀나) + `hot_deal_id`(적용 핫딜) + `order_amount`(스냅샷)를 가진다(현재 `hotDeal`만 참조 → **product 참조 추가**). 1인 1개 유니크는 `hot_deal_id` 기준 유지.
- **API = 핫딜 주소** (우리 읽기와 일관): 구매는 최상위 `POST /api/orders` 에 **`{hotDealId, quantity}`** 를 본문으로 실어 핫딜을 지정한다. 읽기 API 가 핫딜 중심(`GET /api/hotdeals/{id}`)이고 거부 케이스도 핫딜 주소이며, 클라이언트가 상세에서 받은 `hotDealId` 를 그대로 쥐고 사는 흐름과 일관. REST 상 주문은 사용자가 소유하고 `order_no` 로 독립 조회되며 핫딜 취소 후에도 PAID 주문이 유효([ADR-0007 결정2](0007-hotdeal-state-operations.md))해 핫딜 종속 하위 리소스가 아니므로, `POST /api/hotdeals/{id}/orders` 가 아니라 최상위 `/orders` + 본문 참조가 맞다(Stripe·Shopify·commercetools 공통).
- **왜 "상품 주소"가 아닌가**: 선착순 주문 API 주소는 표준이 강제하지 않는다(activity 주소·product 주소 둘 다 실재 — 아키텍처 선택). 상품 주소면 "상품→활성 핫딜 해소" 간접층 + `NO_ACTIVE_DEAL` + 읽기까지 상품 카탈로그로 재편해야 해 스코프가 번진다. 선착순의 "분리"는 dedicated Flash Sale **Service**(인프라)이지 클라이언트 API 분리가 아니고(Thoughtworks/commercetools), 본 프로젝트엔 통합할 일반 주문 API 가 없어 "상품 중심 통합" 자체가 성립하지 않는다 — 읽기·쓰기 모두 **핫딜 주소**로 일관.
