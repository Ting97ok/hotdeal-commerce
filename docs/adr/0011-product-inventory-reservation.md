...
- 관련 규칙: [ADR-0009 재고 동시성](0009-stock-concurrency-design.md) · [ADR-0004 예약 생명주기](0004-stock-reservation-lifecycle.md) · [erd](../design/erd.md) · 실측 출처: 설계노트 하단

## 결정 요약

- **무엇을**: 상품 재고를 `Product` 칼럼이 아닌 별도 `ProductStock`(실물·예약·가용) 테이블에 원본으로 보관하고, 핫딜은 거기서 예약으로 떼어 판다. 핫딜 레벨 재고 행은 `HotDealStock`.
- **왜**: 경합하는 수량을 상품 정보 행에 섞으면 단일 행 쓰기 한계(~500 QPS)에 그대로 부딪힌다. 핫딜 수량의 "출처"도 실재 상품 재고에서 떼어 와야 모델이 성립한다.
- **버린 대안**: ① `Product`에 재고 칼럼 직접 — [ADR-0009](0009-stock-concurrency-design.md)가 떼어낸 행 경합·낙관락 가짜 충돌을 상품 행에서 재현 / ② 가용을 칼럼으로 저장 — 실물·예약이 바뀔 때마다 같이 고쳐야 하고 어긋나면 버그(계산값으로 둠)
- **트레이드오프**: 얻음 = 경합 격리 · 정확한 가용(ATP) 계산 · 회차 간 초과 배정 방지 ↔ 잃음 = 조인/조회 한 번 추가, 테이블 증가
- **동시성**: `ProductStock`은 원자적 조건부 UPDATE 한 문장(낙관락 폐기), `HotDealStock`은 `@Version` 유지(동시성 벤치마크 대상이라)
- **가역성**: **Type1**(되돌리기 어려움) — 스키마·데이터 결정이라 한 번 깔면 비쌈. 그래서 RFC급 근거를 아래 설계노트에 남긴다.

## 설계노트

> 결정에 이른 근거·대안·동작 원리·변경 이력. **결정 자체는 위 요약이 정본**이고, 아래는 "왜 그렇게 결정했나"를 보존한다.
...
- **2026-06-20 · 구매 API 주소를 핫딜 주소 → 상품 주소로 뒤집음.** 아래 "관련 방향"의 근거 참고. 데이터 모델(상품 참조 + 적용 핫딜 기록)은 처음부터 변경 없음 — 바뀐 것은 API 주소뿐.
- **2026-06-20 · 결정 4 동시성을 낙관락 → 원자적 조건부 UPDATE 로 전환.** 저경합 경로에서 재시도·PC 오염을 피하려는 것.
