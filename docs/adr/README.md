...
> 수록 기준은 **"6개월 뒤 새 멤버가 'X 왜 썼어요?' 물을 결정"** 이다. (주제 단위로 전환 중 — 재고 동시성·주문·결제·핫딜이 새 형식이고 나머지 번호 문서는 순차 전환.)
...
|---|---|---|---|
| **Resume-driven design** | "이력서에 쓰려고" 도구 도입 — 문제가 도구를 부르지 않고 도구가 문제를 찾음 | "핵심 흐름 + 다역량 각각 깊게" 정체성 · 전면 MSA 미채택("정당성 없는 분리는 감점") · 5방식 전부 구현 대신 3측정 + 2이론배제("취사선택이 판단력") · k6도 이력서용 아닌 측정 목적(재현성·서버 메트릭)으로 근거 | [0002](0002-monolith-first-partial-msa.md) · [재고 동시성](concurrency.md) · [0013](0013-load-test-tool-k6.md) |
