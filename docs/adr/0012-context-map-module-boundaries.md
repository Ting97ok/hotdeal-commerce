...
- **`stock` 독립** — 재고는 상품/핫딜 *정보* 가 아니라 *경합하는 수량* 이다([ADR-0011](0011-product-inventory-reservation.md) 결정1: 경합 수량을 정보 행에서 떼어낸다). 그 관심사를 코드 모듈에서도 한곳에 모은다 — 3주차 4방식 동시성 벤치마크·정합 검증식이 한 모듈에 응집한다. **층위 구분**: `ProductStock` 의 데이터 *원본 기준* 은 상품 SKU 이지만(ADR-0011), 코드 *모듈* 은 재고 관심사로 묶는다. "상품에 원본 보관"(데이터 모델)과 "stock 모듈 소속"(코드 구조)은 충돌하지 않는다.
