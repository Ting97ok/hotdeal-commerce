...
- **무엇을**: 경계 맥락 **2개**(구매 코어=강한 일관성 / 결제 후속=최종 일관성), 구매 코어 내부는 변경 이유로 가른 **5개 독립 모듈**(product·stock·hotdeal·order·payment). 맥락 간 의존은 **이벤트만**.
- **왜**: 카디널리티(핫딜 1:N 주문)는 경계 맥락을 정하지 않는다 — **변경 이유**로 가른다. MSA 전환 시 경계가 무너지면 부채라, 모놀리식이라도 모듈 경계 = 미래 분리 지점으로 박는다.
- **버린 대안**: 핫딜을 order에 흡수(핵심 차별 영역 core domain이 흐려짐) / 카디널리티로 경계 판단(전략 설계와 집합 단위의 층위 혼동) / context map 무거운 패턴 ACL·Conformist(6도메인엔 과설계)
- **범위 밖(Non-Goal)**: context map 번역·방어 패턴(ACL·Conformist·OHS), 의존 규칙 자동 강제(ArchUnit — 코드 리뷰가 안전장치), 당장의 물리 분리(결제 후속 한 곳만)
- **트레이드오프**: 얻음 = 미래 분리 지점 보존 · core domain 선명 · 의존 규칙이 부채 방지선 ↔ 잃음 = 모듈·조회 경계 오버헤드(단일 저장소엔 당장 이득 없는 선투자)
- **가역성**: **Type2** — 모듈 경계는 패키지 구조라 리팩토링으로 이동 가능(물리 분리 전까지). 진짜 부채 방지선은 패키지 개수가 아니라 **의존 방향**(코어 내부 동기 / 후속은 이벤트)이다.
...
- **`stock` 독립** — 재고는 상품/핫딜 *정보* 가 아니라 *경합하는 수량* 이다([재고 동시성 ADR 1절](concurrency.md) — 경합 수량을 정보 행에서 떼어낸다). 그 관심사를 코드 모듈에서도 한곳에 모은다 — 3주차 5방식 동시성 벤치마크·정합 검증식이 한 모듈에 응집한다. **층위 구분**: `ProductStock` 의 데이터 *원본 기준* 은 상품 SKU 이지만(재고 동시성 ADR), 코드 *모듈* 은 재고 관심사로 묶는다. "상품에 원본 보관"(데이터 모델)과 "stock 모듈 소속"(코드 구조)은 충돌하지 않는다.
