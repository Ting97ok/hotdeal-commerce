...
| 기간 배타 제약 부재 | MySQL은 기간 겹침을 DB 제약으로 표현할 수 없어, 겹침 금지는 등록 API의 서비스 검증으로 막는다. | [주문 ADR 7절](../adr/order.md) · [ADR-0007 결정4](../adr/0007-hotdeal-state-operations.md) |
| 공개 조회 인증 | 조회는 공개이므로 `SecurityConfig`의 `permitAll`에 `GET /api/hotdeals/**`를 추가한다. | — |
| DB CHECK 최후 방어 | 서비스 검증이 뚫려도 데이터 오염을 막는 안전망 — `ck_hot_deals_period`(start_at < end_at) · `ck_hot_deals_total_quantity`(> 0) · `ck_hot_deal_stock_remaining`(remaining_quantity >= 0) · `ck_product_stock_reserved`(reserved_quantity <= on_hand_quantity). | [주문 ADR 7절](../adr/order.md) |
