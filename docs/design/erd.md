...
erDiagram
    ...
    HOT_DEAL ||--|| HOT_DEAL_STOCK : "딜 예약 재고"
    HOT_DEAL ||--o{ ORDERS         : "구매 대상(적용 딜)"
    ...
    PRODUCT_STOCK {
        ...
        int    reservedQuantity "예약(딜에 떼어 둔 수)"
    ...
    ORDERS {
        ...
        bigint   hotDealId FK "논리 참조 — 적용 딜"
...
| **ProductStock** | 상품 재고 **원본**(실물·예약) | 가용=실물−예약(계산, 저장 안 함). 딜 등록이 여기서 **예약**, 결제확정이 실물·예약 차감 — 등록/취소·결제확정(저경합)만 건드려 핫 패스와 분리 ([ADR-0011](../adr/0011-product-inventory-reservation.md)) |
...
| **Order** | 구매 1건(회원 × 핫딜) | 산 상품(`product`) + 적용 딜(`hot_deal`) 참조 · 주문 시점 금액 저장 · 1인 1개 유니크 · 불투명 주문번호 · 선점 만료 시각 · 취소 사유 |
...
| Product → Order | 1 : N | 주문이 산 상품 참조(적용 딜은 `hot_deal`) ([ADR-0011](../adr/0011-product-inventory-reservation.md)) |
| HotDeal → HotDealStock | 1 : 1 | 딜 예약 재고 행 — 경합 격리 ([ADR-0009](../adr/0009-stock-concurrency-design.md)) |
...
| 장부 일치(정합 검증식) | 딜: `totalQuantity = remaining + Σ(활성 주문 qty)` · `주문당 승인 ≤ 1` · `PAID ↔ 승인 1:1` — 쿼리로 검증. 상품: `ProductStock.reserved` 는 활성 딜 예약분 — 변동 원장 정식화는 슬라이스 2 ([ADR-0011](../adr/0011-product-inventory-reservation.md) 보류) |
...
수천 동시 구매 → 동일 HotDeal → 동일 **HotDealStock 1행** 차감. 경합은 이 한 행에 집중되며, 4방식(낙관/비관/Redis/분산락)을 이 한 행·키에 교체 적용해 비교한다(3주차).
...
- **재고 테이블 분리** — `product_stock`(상품 재고 원본): `product_id`(UNIQUE 논리 참조)·`on_hand_quantity`·`reserved_quantity`·`version`. `hot_deal_stock`(딜 예약 재고 — 기존 `stock` 리네임): `hot_deal_id`(UNIQUE)·`remaining_quantity`·`version`. 가용(실물−예약)은 **저장 안 함**(조회 시 계산) ([ADR-0011](../adr/0011-product-inventory-reservation.md)).
- **상품 재고 시드** — `on_hand` 초기값은 시드/픽스처(운영 입고 API 는 스코프 밖 — [ADR-0011](../adr/0011-product-inventory-reservation.md) 보류). Product 생성 시 `product_stock` 행 동반 생성 — 딜 등록 가용검사가 의존하므로 "출처 없음" 재발 방지.
- **orders 칼럼 확정** — `order_no CHAR(36)`(UUID v4) · `product_id BIGINT NOT NULL`(논리 참조 — 산 상품) · `hot_deal_id BIGINT NOT NULL`(논리 참조 — 적용 딜) · `expires_at DATETIME(6) NOT NULL`(임시 10분 — 최종값은 슬라이스 2) · `cancel_reason VARCHAR(30) NULL`(후보: PAYMENT_FAILED·EXPIRED).
...
- **구매 API(슬라이스 1)** — 딜 주소 `POST /api/orders {hotDealId}`(읽기와 일관), `Order` 는 `product`+`hot_deal` 참조 + 금액 스냅샷([ADR-0011](../adr/0011-product-inventory-reservation.md) 관련 방향).
