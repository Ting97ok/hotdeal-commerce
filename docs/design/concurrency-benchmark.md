...
현 측정은 **앱 1인스턴스**다. 배포 전제는 서버 여러 대이므로([기술 가설 3장](hotdeal-purchase-hypothesis.md)), 다중 인스턴스에서만 드러나는 것을 별도로 측정한다.

**구성** — 기존 격리 스택을 거의 그대로 재사용한다.

```
k6 → nginx(LB) → 앱 컨테이너 ×3 → MySQL 1 + Redis 1
```

[benchmark/docker-compose.yml](../../k6/benchmark/docker-compose.yml)의 앱 서비스를 3개로 늘리고 nginx upstream 을 추가하면 되며, `run.sh` 의 전략 순회·정합 검증 로직은 그대로 쓴다.

**무엇을 보나** — 1인스턴스 측정에서는 나오지 않는 것들:

| 관측 | 확인하려는 것 |
|---|---|
| 오버셀 0 유지 | "DB 가 직렬화하니 인스턴스 수와 무관"이라는 **설계 주장의 실증**(현재는 논리적 추론에 머묾) |
| 인스턴스 1 → 3 처리량 변화 | **비례하지 않을 것**(단일 행 천장) — [nfr 다중 인스턴스 전제](nfr.md)의 "스케일아웃해도 재고 천장은 안 풀린다"를 수치로 |
| DB 커넥션 총량 | 인스턴스별 `hikaricp_connections_active` 합산 vs DB `Threads_connected` — N×풀 산정 검증 |
| 스케줄러 중복 | 같은 주기에 N개가 동시 실행되는 양상과 외부 호출 횟수([payment system 설계](../payment/api-design-system.md) 처리권 선점의 근거) |

**기대 결과**: 처리량이 인스턴스 수에 비례하지 않고 DB 단일 행에서 막히는 것 — "스케일아웃이 만능이 아니다"를 실측으로 보이는 자료가 된다.
